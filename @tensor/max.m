...
%TENSOR/MAX Implement max for tensor objects.
...
switch nargin
  ...
  case 2
    ...
    switch wscalar
      case 0
        c = max(varargin{1}.data,varargin{2}.data);
        %a.dimension(1),b.dimension(1) are equal or one of them is zero.
        c = tensor(c, ...
          max(rawfielddim(varargin{1}),rawfielddim(varargin{2})));
      case 1
        c = max(varargin{1},varargin{2}.data);
        c = tensor(c, rawfielddim(varargin{2}));
      case 2
        c = max(varargin{1}.data,varargin{2});
        c = tensor(c,rawfielddim(varargin{1}));
      otherwise
